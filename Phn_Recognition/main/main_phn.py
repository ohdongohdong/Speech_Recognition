...
# Filename     : main_phn.py
# Description  : Training models on TIMIT dataset for Phoneme Recognition
#
# Data : TIMIT
...
flags.DEFINE_integer('num_class', 39, 'set the size of num phn')
flags.DEFINE_integer('num_epoch', 100, 'set the number of epochs')
...
flags.DEFINE_string('datadir', '../data/timit_phn', 'set the data root directory')
...
data_dir = os.path.join(datadir, mode)
    ...
    def test(self, args, model, input_test, target_test, seq_test):
        ...
        with tf.Session(graph=model.graph, config=get_tf_config()) as sess: 
            ...
            for epoch in range(num_epoch):
                ...
                total_truth = np.asarray(total_truth)
                cm_name = os.path.join(resultdir, 'cm.png')
                cm = confusion_matrix(total_truth, total_pred, resultdir)
